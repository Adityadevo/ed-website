...
export default function ContactSection() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://formspree.io/f/xjggpzld', {
        method: 'POST',
        headers: {
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="flex items-center gap-10 mt-2 my-5">
              <Link
                ...
                aria-label="Facebook"
                className="w-12 h-12 grid place-items-center bg-white/10 rounded-sm hover:bg-white/20 transition"
              >
          ...
          <div className="bg-white rounded-2xl border shadow-md overflow-hidden">
            <div className="bg-[#E9CDBB] text-[#4C231A] px-7 py-6 flex items-center gap-3">
              ...
              <p className="font-medium text-lg">
                Write us a few words about your concerns and queries and our
                team shall reach out to you
              </p>
            </div>
            <form
              onSubmit={onSubmit}
              className="p-7 grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6"
            >
              <div className="flex flex-col gap-1">
                <input
                  ...
                  placeholder="Name *"
                  className={`w-full rounded-md border px-4 py-3.5 text-lg bg-[#FBF7EE] placeholder:text-[#876] focus:outline-none focus:ring-2 focus:ring-[#7A4638] ${
                    errors.name ? "border-red-400" : "border-[#E7D8C8]"
                  }`}
                />
                {errors.name && (
              ...
              <div className="flex flex-col gap-1">
                <input
                  ...
                  className={`w-full rounded-md border px-4 py-3.5 text-lg bg-[#FBF7EE] placeholder:text-[#876] focus:outline-none focus:ring-2 focus:ring-[#7A4638] ${
                    errors.email ? "border-red-400" : "border-[#E7D8C8]"
                  }`}
                />
                {errors.email && (
              ...
              <div className="md:col-span-2 flex flex-col gap-1">
                <textarea
                  ...
                  rows={6}
                  className={`w-full rounded-md border px-4 py-3.5 text-lg bg-[#FBF7EE] placeholder:text-[#876] focus:outline-none focus:ring-2 focus:ring-[#7A4638] ${
                    errors.message ? "border-red-400" : "border-[#E7D8C8]"
                  } `}
                />
                {errors.message && (
